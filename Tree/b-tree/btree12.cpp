...
bnode * insertnsort(bnode *T,int k)
{
	int i,j;
...
bnode* splitnconnect(bnode *T,int k)
{
//	z++;
//	if(z==6)
//		return;
...
//		levelorder(X);
	...
	cout<<"\n its child are "<<T->child[0]->data[0]<<T->child[0]->data[1]<<"   "<<T->child[1]->data[0]<<T->child[1]->data[1]<<endl;
	return T;
}
				...
				for(i=0;i<T->count;i++)
				{
					...
					else
					{
						...
						if(T->child[i]!=NULL)
							s.push(T);
						cout<<"\npushed node is "<<s.top()->data[0];
						T->child[i+1]=add(T->child[i+1],k,1);
						break;
						}
					}
				}
