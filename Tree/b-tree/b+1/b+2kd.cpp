...
void keydistribution(spnode *&S,int dest,int source, int leaf)
{
	if(leaf)
	{
		if(dest>source)
...
//		X->child[source]->count--;
		X->data[source]=X->child[dest]->data[0];
	}
	else
	{
...
void splitnconnect(bnode *&T,int k,int leaf)
{
	if(leaf==1)
	{
